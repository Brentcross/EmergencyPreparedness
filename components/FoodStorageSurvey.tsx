...
import { foodStorageQuestionGroups, defaultFoodStorageInput } from "@/lib/questions";
...
export default function FoodStorageSurvey() {
  const [input, setInput] = useState<FoodStorageInput>(defaultFoodStorageInput);
  const [step, setStep] = useState(1);
  const [result, setResult] = useState<FoodStorageResult | null>(null);

  const totalSteps = foodStorageQuestionGroups.length + 1;
  const currentGroup = foodStorageQuestionGroups[step - 1];

  const handleChange = (id: string, value: string | number) => {
    setInput((current) => ({ ...current, [id]: value } as FoodStorageInput));
  };

  const handleStapleChange = (label: string, value: number) => {
    setInput((current) => ({
      ...current,
      longerTermStaples: {
        ...current.longerTermStaples,
        [label]: value,
      },
    }));
  };
  ...
  const stepReady = useMemo(() => {
    ...
    return currentGroup.items.every((item) => {
      const value = item.type === "number" ? (input.longerTermStaples[item.label] ?? (input as unknown as Record<string, unknown>)[item.id]) : (input as unknown as Record<string, unknown>)[item.id];
      return value !== undefined && value !== "";
    });
  }, [currentGroup, input]);

  const handleNext = () => {
    if (step < foodStorageQuestionGroups.length) {
      setStep(step + 1);
      window.scrollTo({ top: 0, behavior: "smooth" });
    } else {
      const computed = computeFoodStorageScore(input);
      setResult(computed);
      window.localStorage.setItem("latestFoodStorageResult", JSON.stringify(computed));
      window.localStorage.setItem("latestFoodStorageInput", JSON.stringify(input));
    }
  };
    ...
    <main className="mx-auto max-w-6xl px-6 py-10 lg:px-8">
      <section className="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <p className="text-sm uppercase tracking-[0.24em] text-slate-500">Food Storage Assessment</p>
            <h1 className="mt-3 text-3xl font-semibold text-slate-900">Practical storage for your family</h1>
          </div>
          <Link href="/" className="rounded-full bg-slate-100 px-4 py-2 text-sm font-semibold text-slate-800 hover:bg-slate-200">
            Back to home
          </Link>
        ...
        <p className="mt-6 max-w-3xl text-slate-700">
          Track your short-term food and water supplies, longer-term staples, and rotation habits with a focus on gradual, sustainable progress.
        </p>
            ...
            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-6">
              ...
              <div className="mt-6 space-y-6">
                {currentGroup.items.map((item) => {
                  const isStaple = item.type === "number" && item.id !== "householdSize" && item.id !== "daysNormalFood" && item.id !== "waterGallonsPerPerson";
                  const value = isStaple ? input.longerTermStaples[item.label] ?? 0 : (input as unknown as Record<string, unknown>)[item.id] ?? "";
                  return (
                      ...
                      {item.type === "number" ? (
                        <input
                          ...
                          onChange={(event) => {
                            ...
                            isStaple ? handleStapleChange(item.label, newValue) : handleChange(item.id, newValue);
                          }}
                        ...
                        <div className="grid gap-3 sm:grid-cols-3">
                          {item.options?.map((option) => {
                            const label = radios[option as keyof typeof radios] ?? option;
                            return (
        ...
        <div className="mt-8 flex flex-wrap items-center gap-3">
          ...
          >
            {step === foodStorageQuestionGroups.length ? "Review results" : "Next"}
          </button>
